...
bool TerrainTinRenderer::InitWireframe(md::GpuDeviceHandle /*dev*/) {
    GpuPipeline::Desc pd;
    pd.layout.count      = 2;
    pd.layout.stride     = 24;
    pd.layout.attribs[0] = { 0, 0,  GpuAttribFmt::F3 };
    ...
    pd.raster.depth_test  = true;
    // depth_write=true: PRIMARY content here (no shaded pass runs first to
    // populate depth, unlike TerrainQuadtreeRenderer's own DEBUG overlay
    // wireframe_pipeline_), needed so the quadtree's batched-wireframe tiles
    // correctly occlude/are occluded by this zone's TIN mesh.
    pd.raster.depth_write = true;
    ...
    pd.frag_samplers     = 0;
    pd.frag_storage_bufs = 0;
    // color_format explicit -- this method is editor-only (no game call
    // site) and the editor's 3D World tab draws into an OFF-SCREEN RTT
    // (s_color, editor_world_3d_sdlgpu.cpp's ensure_rtt), NOT the
    // swapchain. See TerrainQuadtreeRenderer::InitBatchedWireframe's own
    // doc comment for the VK_ERROR_DEVICE_LOST this fixes (confirmed via
    // VK_LAYER_KHRONOS_validation's VUID-vkCmdDraw-renderPass-02684).
    pd.color_format = SDL_GPU_TEXTUREFORMAT_R8G8B8A8_UNORM;
    if (!wireframe_pipeline_.Create(pd)) {
        MD_LOG(MD_LOG_WARNING, "[TerrainTinRenderer] wireframe pipeline create failed");
        return false;
    }
...
void TerrainTinRenderer::DrawMeshWireframe(SDL_GPURenderPass* rp, md::GpuCommandBufferHandle cmd,
                                            const TerrainTinMesh& mesh, const float* vp16,
                                            float zone_origin_x, float zone_origin_z,
                                            float cam_x, float cam_y, float cam_z) {
    if (!wireframe_ready_ || !mesh.IsReady()) return;
    if (!mesh.vbo.SDLBuffer() || !mesh.ibo.SDLBuffer()) return;
