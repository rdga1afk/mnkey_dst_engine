...
| Terrain geometry + shading | **Default:** flat fixed-depth tiling (`terrain_quadtree.vert/.frag`, plain GLSL — Slang removed 2026-07-26; geometry simplified 2026-09-04): `TerrainQuadtree::SelectVisible` walks a fixed depth (`kFlatLodDepth=3`) out to `kFlatMaxRenderDistance=3000 m` — no adaptive subdivision, no geomorph blend, no skirts, no stitched-IBO seam handling (removed after RMSE-verified pixel-identical output at 2.3–3.6× lower render cost than the old adaptive quadtree); one shared filled index buffer per node depth. **Actively-developed replacement (off by default):** `TerrainProjectedGrid` (TPG, since 2026-09-12) — fixed screen-space projected grid, `md.set_terrain_projected_grid(true)` runtime toggle; wide-scan-confirmed regressions not yet fixed (single `y_base` plane poorly models relief on shallow far rays, error amplified ~4× at gameplay-pose grazing angles) — kept and being redesigned, NOT scheduled for deletion; `TerrainQuadtree` is the actual long-term deletion candidate once TPG's redesign clears re-verification. Ground shading (`TerrainShadingProjected`, screen-space G-buffer resolve) is shared and unaffected by which geometry system is active; matches original Kenshi (same material at all tiers, no POM/normal-mapping split), per-pixel dominant-weight selection (base/slope/cliff/grass/dirt/road) |
...
- **Flat fixed-depth LOD** (2026-09-04, replaces the earlier adaptive quadtree + `BuildLodIboStitched()` seam-stitch) — every visible node renders at the same fixed depth (`kFlatLodDepth=3`), so the T-junction/seam problem the old stitched-IBO approach solved no longer arises by construction; validated pixel-identical (RMSE=0.0) against the old adaptive output across 4 diverse locations before the ~300-line adaptive-subdivision/geomorph/skirt code path was deleted. **This is `TerrainQuadtree`, the current runtime default — but the long-term deletion candidate**, see `TerrainProjectedGrid` below.
- **`TerrainProjectedGrid`** (TPG, `terrain_projected_grid.cpp`, since 2026-09-12) — fixed screen-space projected grid, the actively-developed intended replacement for `TerrainQuadtree` (off by default via `terrain_projected_grid_` flag, `md.set_terrain_projected_grid(true)` runtime toggle). Two regression mechanisms found by an 18-zone×2-pose wide scan (2026-09-13): corner-cap threshold flapping on mountainous relief (up to +84% GPU cost), and a stable regression (+27-49%) root-caused to the single `y_base` plane poorly modelling non-flat relief — a shallow far ray's height error gets amplified ~4× at gameplay-pose grazing angles, producing degenerate/inconsistent triangles (visually: sky bleeds in at the true screen edges). Kept and scheduled for redesign, not deletion, per owner direction the same day.
